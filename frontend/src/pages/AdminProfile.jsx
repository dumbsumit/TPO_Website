...
import {
  Shield, User, Mail, Lock, Save, Eye, EyeOff,
  ArrowLeft, CheckCircle, KeyRound, Send, RefreshCw,
} from "lucide-react";
...
export default function AdminProfile() {
  ...
  // OTP flow: "form" → "otp" → (back to "form" on success)
  ...
  const handleRequestOTP = async (e) => {
    ...
    setOtpSending(true);
    try {
      const res = await fetch(`${API_URL}/auth/admin/request-password-otp`, {
        method: "POST",
        credentials: "include",
      });
      const data = await res.json();
